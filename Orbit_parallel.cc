...
int main(int argc,char *argv[])  
{
 ...
 file.close();
  ...
  // read file
    ...
    Vector<double,4> *EVL = new Vector<double,4> [nData];
    ...
    for (int i=0; i<nData; i++) {
        ...
        OI.runWithOutputIncludingTime(OrbOut,tOut,nOut);
            ...
            OVL[i][6]=tOut[nOut-1];
        EVL[i][0]=OI.Minr/Units::kpc;
        EVL[i][1]=OI.Maxr/Units::kpc;
        EVL[i][2]=(OI.Maxr-OI.Minr)/(OI.Maxr+OI.Minr);
        EVL[i][3]=OI.Energy/(Units::kms*Units::kms);
        EVL[i][4]=OI.Lz/(Units::kpc*Units::kms);
